...
if((_vInfo select 1) == "adac" && (_vInfo select 2) in ["C_Offroad_01_F","B_G_Offroad_01_F"]) then
{
	[[_vehicle,"adac_offroad",true],"life_fnc_vehicleAnimate",_unit,false] spawn life_fnc_MP;
};
[[1,"Dein Fahrzeug ist bereit!"],"life_fnc_broadcast",_unit,false] spawn life_fnc_MP;
serv_sv_use = serv_sv_use - [_vid];