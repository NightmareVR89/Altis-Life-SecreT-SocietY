...
if(_ctrl != "goldbar") exitWith {hint "Du kannst hier nichts ausser Goldbarren lagern"};
if(_num > _safeInfo) exitWith {hint format["Es sind keine %1 Goldbarren in dem Safe!",_num];};
...
//Take it
if(!([true,_ctrl,_num] call life_fnc_handleInv)) exitWith {hint "Konnte nichts in deinem Inventar lagern.";};
