...
if(life_inv_niere >= 2) exitWith {hint "Es ist unwahrscheinlich das Du so viele Organe brauchst!"};//if you already have 2 kidneys, then go sell them already, no stockpiling
...
while{true} do
		{
			...
			_pgText ctrlSetText format["%3 (%1%2)...",round(_cP * 100),"%"];
			if(_cP >= 1) exitWith {};
