...
if(_sum > 0) then
{
	...
	titleText[format[(localize "STR_NOTF_Gathering"),localize "STR_Item_Tabaks"],"PLAIN"];
	player playMove "AinvPercMstpSnonWnonDnon_Putdown_AmovPercMstpSnonWnonDnon";
	player playMove "AinvPercMstpSnonWnonDnon_Putdown_AmovPercMstpSnonWnonDnon";
	sleep 5;
	if(([true,"tabak",_sum] call life_fnc_handleInv)) then
	{
		titleText[format[(localize "STR_NOTF_Picked"),_sum,localize "STR_Item_Tabaks"],"PLAIN"];
	};
}
	else
{
