...
if(isNil {grpPlayer getVariable "gang_name"}) exitWith {titleText["Vor dem Einnehmen musst du eine Gang gründen!","PLAIN"];};
if(_group == grpPlayer) exitWith {titleText["Deine Gang kontrolliert dieses Gebiet bereits!","PLAIN"]};
if((_hideout getVariable ["inCapture",FALSE])) exitWith {hint"Es kann immer nur eine Person den Hideout einnehmen";};
if(!isNull _group) then {
	_gangName = _group getVariable ["gang_name",""];
	_action = [
		format["Dieses Gebiet wird von %1 kontrolliert<br/>Bist du Sicher, dass du deren Ganggebiet einnehmen willst?",_gangName],
		"Dieses Gebiet gehört bereits jemandem..",
		"Ja",
		"Nein"
...
titleText["Ganggebiet wurde eingenommen.","PLAIN"];
_flagTexture = [
...
[[[0,1],format["%1 und seine Gang: %2 - haben die Kontrolle über ein lokales Gangversteck erlangt",name player,(group player) getVariable "gang_name" ]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
