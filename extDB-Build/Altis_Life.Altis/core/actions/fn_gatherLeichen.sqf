...
if(_sum > 0) then
{
	...
	player say3D "take";
	titleText[format[(localize "STR_NOTF_Gathering"),localize "STR_Item_Schmucks"],"PLAIN"];
	player playMove "AinvPercMstpSnonWnonDnon_Putdown_AmovPercMstpSnonWnonDnon";
	sleep 2;
	if(([true,"schmuck",_sum] call life_fnc_handleInv)) then
	{
		titleText[format[(localize "STR_NOTF_Picked"),_sum,localize "STR_Item_Schmucks"],"PLAIN"];
	};
}
	else
{
