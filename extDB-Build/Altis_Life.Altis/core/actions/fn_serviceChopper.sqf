...
_pgText ctrlSetText format["Repariere Helikopter (1%1)...","%"];
...
while {true} do
{
	...
	_pgText ctrlSetText format["Repariere Helikopter (%1%2)...",round(_cP * 100),"%"];
	if(_cP >= 1) exitWith {};
};

if(!alive (_search select 0) || (_search select 0) distance air_sp > 10) exitWith {life_action_inUse = false; hint "Das Fahrzeug ist kaputt, oder nicht mehr auf dem Landeplatz!"};
