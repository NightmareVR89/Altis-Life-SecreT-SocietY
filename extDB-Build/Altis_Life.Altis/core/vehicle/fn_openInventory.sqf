...
if((_vehicle getVariable ["trunk_in_use",false])) exitWith {hint "Dieser Kofferraum wird gerade benutzt, es kann immer nur eine Person darauf zugreifen."};
...
if(_vehicle isKindOf "House_F") then {
	ctrlSetText[3501,format["Lagerraum - %1",getText(configFile >> "CfgVehicles" >> (typeOf _vehicle) >> "displayName")]];
} else {
