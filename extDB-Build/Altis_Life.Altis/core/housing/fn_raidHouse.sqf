...
if(!([_uid] call life_fnc_isUIDActive)) exitWith {hint "Die Person ist nicht online, deshalb kann ihr Haus nicht durchsucht werden!"};
_houseInv = _house getVariable ["Trunk",[[],0]];
if(_houseInv isEqualTo [[],0]) exitWith {hint "In diesem Haus gibt es nichts."};
life_action_inUse = true;

//Setup the progress bar
disableSerialization;
_title = "Durchsuche Haus....";
...
5 cutText ["","PLAIN"];
if(player distance _house > 13) exitWith {life_action_inUse = false; titleText["Du bist zu weit vom Haus entfernt!","PLAIN"]};
...
if(_value > 0) then {
	[[0,format["Ein Haus wurde durchsucht und es wurden Drogen / Schmuggelwaren im Wert von $%1 gefunden.",[_value] call life_fnc_numberText]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	...
	hint "In diesem Haus ist nichts illegales gelagert.";
};